...
bluewards_ind = rest_wavelengths_testing(rest_wavelengths_testing < min_lambda);
redwards_ind = rest_wavelengths_testing(rest_wavelengths_testing > max_lambda);
mu = [ ones(size(bluewards_ind))*bluewards_fluxes_mean, ...
       mu, ...
       ones(size(redwards_ind))*redwards_fluxes_mean ];
M = [ zeros(length(bluewards_ind), k);...
      M; ...
      zeros(length(redwards_ind), k)];
log_omega = [ ones(size(bluewards_ind))*bluewards_fluxes_var, ...
       log_omega, ...
       ones(size(redwards_ind))*redwards_fluxes_var ]; 
...
for quasar_ind = q_ind_start:num_quasars %quasar list
    ...
    for i = 1:num_dla_samples       %variant redshift in quasars 
        ...
        this_noise_variance = [ zeros(size(bluewards_ind)), ...
        this_noise_variance, ...
